...
const Shop = () => {
    ...
    useEffect(()=>{
     const getProductsBySubCategory=async()=>{
        try {
         dispatch(getProductsBySubCategoryAction(id))
          ...
          {filteredProduct.map((product) => (
            ...
            <div className="card product-item border-0 mb-4">
              <div className="card-header product-img position-relative overflow-hidden bg-transparent border p-0">
                <img className="img-fluid w-100" src={`http://localhost:3001/uploads/images/${product.image}`} alt={product.name} />
              </div>
              <div className="card-body border-left border-right text-center p-0 pt-4 pb-3">
                <h6 className="text-truncate mb-3">{product.name}</h6>
                <div className="d-flex justify-content-center">
                  <h6>${product.price}</h6><h6 className="text-muted ml-2"><del>${(product.price * 1.2).toFixed(2)}</del></h6>
                </div>
              </div>
